...
async function getLearnedCategory(merchant: string): Promise<string | null> {
  const learned = await MerchantLearningService.getLearnedCategory(merchant);
  if (learned) return learned;

  // Legacy Preferences key from older builds that never wrote MerchantLearningService
  const { value } = await Preferences.get({ key: 'category_corrections' });
  if (!value) return null;
  try {
    const corrections: Array<{ merchant: string; category: string; timestamp: string }> =
      JSON.parse(value);
...
export async function requestSmsPermissions(): Promise<boolean> {
  try {
    ...
    if (
      status &&
      (status.read === 'granted' || status === 'granted' || status.messages === 'granted')
    ) {
...
export async function checkSmsPermissionsOnly(): Promise<boolean> {
  try {
    const status = await SMSReader.checkPermissions();
    return (
      status && (status.read === 'granted' || status === 'granted' || status.messages === 'granted')
    );
  } catch {
    return false;
  }
...
export async function parseSmsToTransaction(
  ...
  if (!/(sent|spent|debited|credited|received|paid|withdrawn|transferred)/i.test(smsBody))
    return null;
  ...
  if (toMatch) {
    ...
    if (vpaMatch) {
      ...
      if (genericMatch) {
        ...
        if (
          /^\d{10}$/.test(merchant) ||
          merchant.includes('7308080808') ||
          merchant.includes('18002586161')
        ) {
          merchant = 'Unknown';
        }
      }
  ...
  if (!learned) {
    ...
    if (category === 'Other' && geminiService) {
      ...
      try {
        ...
        log.error('smsService', 'Gemini error', e);
      }
    }
  }
...
export async function fetchAllSmsTransactions(): Promise<Transaction[]> {
  ...
  try {
    ...
    if (lastSyncTime > 0) {
      ...
      log.info(
        'smsService',
        `🕒 Incremental Sync: Fetching SMS after ${new Date(fetchFrom).toLocaleString()}`
      );
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const tx = await parseSmsToTransaction(
        msg.body,
    ...
    if (unknownTransactions.length > 0 && geminiService) {
      log.info(
        'smsService',
        `🤖 AI Categorizing ${unknownTransactions.length} unknown transactions...`
      );
