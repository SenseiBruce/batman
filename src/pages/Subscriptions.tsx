...
import {
  ...
  deleteSubscription,
  addSubscription,
} from '../services/subscriptionStorageService';
import { scheduleSubscriptionReminder } from '../utils/notify';
import { formatSubscriptionSummary } from '../utils/subscriptionSummary';
...
const Subscriptions: React.FC<SubscriptionsProps> = ({ transactions }) => {
  ...
  const saveEdit = async () => {
    ...
    const updated: Subscription = {
      ...
      lastPaymentDate:
        subs.find((s) => s.id === editingId)?.lastPaymentDate || new Date().toISOString(),
  ...
  const copySummary = async () => {
    try {
      await navigator.clipboard.writeText(formatSubscriptionSummary(subs));
      setCopied(true);
        ...
        <div className="flex items-center gap-2">
          {subs.length > 0 && (
            <button
              type="button"
              onClick={copySummary}
              className="px-3 py-1 bg-gray-700 hover:bg-gray-600 text-white rounded text-sm font-medium inline-flex items-center gap-1"
            >
        ...
        <div className="bg-gray-800 p-4 rounded-xl border border-blue-500 mb-4 animate-fade-in">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2 mt-2">
              <button
                onClick={saveNew}
                className="flex-1 py-1 bg-green-600 hover:bg-green-500 text-white rounded"
              >
                Save
              </button>
              <button
                onClick={cancelAdd}
                className="flex-1 py-1 bg-gray-600 hover:bg-gray-500 text-white rounded"
              >
                Cancel
              </button>
            </div>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-gray-400 text-sm">
                      ₹{sub.amount.toLocaleString()} • every {sub.frequencyDays} days
                    </p>
                  </div>
