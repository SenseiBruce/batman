...
interface TransactionReviewModalProps {
  ...
  onApproveAll: (transactions: Transaction[]) => void;
  onDiscard?: (transaction: Transaction) => void;
}
...
export const TransactionReviewModal: React.FC<TransactionReviewModalProps> = ({
  ...
  const handleApproveAll = () => {
    ...
    const approvedTxs = localPending.map((tx) => {
      const finalCategory =
        editingTx === tx.id && selectedCategory ? selectedCategory : tx.category;
      return {
        ...tx,
    ...
    // Call parent's bulk approve handler
    onApproveAll(approvedTxs);

    // Clear local list immediately for instant visual feedback and smooth exit animations
    setLocalPending([]);
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 bg-black/80 flex items-end justify-center z-50 backdrop-blur-sm"
        >
          <motion.div
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            ...
            <div className="flex justify-between items-center mb-6 sticky top-0 bg-gray-900 z-10 pb-4 border-b border-gray-800">
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  {localPending.length} transaction{localPending.length !== 1 ? 's' : ''} pending
                </p>
              </div>
              <div className="flex items-center gap-3">
                {localPending.length > 1 && (
                  <button
                    ...
                    className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white px-3 py-1.5 rounded-xl text-xs font-semibold shadow-lg shadow-green-900/20 transition-all active:scale-95 flex items-center gap-1"
                  >
                    ✓ Approve All
                  </button>
                )}
                  ...
                  localPending.map((tx, index) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <div
                          className={`text-lg font-bold ${tx.type === 'credit' ? 'text-green-400' : 'text-white'}`}
                        >
                      ...
                      <div className="flex gap-3 items-center mt-4">
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <span className="text-lg">
                                  {categories.find((c) => c.name === tx.category)?.icon || '🏷️'}
                                </span>
                                <span className="text-sm text-gray-300">{tx.category}</span>
                              </div>
                              <span className="text-gray-500 text-xs group-hover:text-white transition-colors">
                                Edit
                              </span>
                            </button>
                          )}
                        ...
                        {onDiscard && (
                          <button
                            ...
                            className="bg-gray-700 hover:bg-gray-600 text-gray-200 px-3 py-2 rounded-lg flex items-center justify-center min-w-[64px] h-[42px] transition-colors font-medium text-sm"
